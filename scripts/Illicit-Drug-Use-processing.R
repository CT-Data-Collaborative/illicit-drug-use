...
##################################################################
#
# Processing Script for Illicit Drug Use
...
for (i in 1:length(legacy_table_dfs)) {
  ...
  descriptions <- c("Illicit Drug Use in the Past Month", 
...
new_table_dfs <- new_table_dfs[grepl(paste(c(" 1$", " 6", " 17", " 18"), collapse = "|"), new_table_dfs)]
...
test <- test[grepl("Illicit", test$`Table Description`),]

#recode description column
test$`Illicit Drug Use` <- NA
test$`Illicit Drug Use`[grep("Month", test$`Table Description`)] <- "Illicit Drug Use in the Past Month"
test$`Illicit Drug Use`[grep("Other", test$`Table Description`)] <- "Illicit Drug Use Other Than Marijuana in the Past Month"
test$`Illicit Drug Use`[grep("Dependence", test$`Table Description`)] <- "Illicit Drug Dependence in the Past Year"
test$`Illicit Drug Use`[grep("Abuse", test$`Table Description`)] <- "Illicit Drug Dependence or Abuse in the Past Year"
...
#assign variable
all_data_long$Variable[grep("Estimate", all_data_long$Variable)] <- "Illicit Drug Use"
...
all_data_final <- all_data_long %>% 
  select(`Substate Region`, Year, `Age Range`, `Illicit Drug Use`, `Measure Type`, Variable, Value) %>% 
  rename(Region = `Substate Region`) %>% 
  arrange(Region, Year, `Age Range`, `Illicit Drug Use`, Variable)

# Write to File
write.table(
  all_data_final,
  file.path(getwd(), "data", "illicit-drug-use-2014.csv"),
  sep = ",",
  na = "-9999",
  row.names = F
)
